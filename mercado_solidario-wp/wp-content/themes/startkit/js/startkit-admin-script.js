( function( $ ){
    $( document ).ready( function(){
      $( '.startkit-btn-get-started' ).on( 'click', function( e ) {
          e.preventDefault();
          $( this ).html( 'Processing.. Please wait' ).addClass( 'updating-message' );
          $.post( startkit_ajax_object.ajax_url, { 'action' : 'install_act_plugin' }, function( response ){
        ...
        $.ajax( ajaxurl,
          {
            type: 'POST',
            data: {
              action: 'startkit_dismissed_notice_handler',
              type: type,
            }
          } );
      } );
